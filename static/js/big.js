window.onload = function() {
  ...
  for (i = 0; i < s.length; i++) {s[i].setAttribute('tabindex', 0); s[i].setAttribute('id',i);}
  if (!s.length) return;
  ...
  function go(n) {
    ...
    if (e.className === 'commalist') {e.innerHTML = e.innerHTML.split(',').map(function(i){return "<span>"+i+"</span>";}).join(',');}
  }
